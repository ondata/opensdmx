...
| ISTAT | ✓ | ✓ | Use `--provider istat`; `constraints` uses the StatKit Data Browser hub by default — sub-second per dimension, exposes every dim including `REF_AREA`. Dedicated reference: [istat-flow.md](istat-flow.md) |
