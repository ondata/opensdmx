...
- **Read for every provider, no extra call**: new catalogue column `df_sdmx_description` (null where absent, backfilled null on older caches like the annotation columns). `load_dataset` exposes `notes` = `DATAFLOW_NOTES` then the Description, only those present; `info` prints them as `Note:` lines (`notes` in JSON) and `embed` folds the text into the document like `df_notes`.
- **Time coverage probed on all 15 providers, one call each** (`tmp/ac_*.xml`): three positive shapes — `TimeRange` (istat `availableconstraint`, oecd, abs), Start/End pair in the hub `TIME_PERIOD` criterion (istat, inps), explicit value list (eurostat `contentconstraint`) — and five ways of not answering (200 without `TIME_PERIOD`: bis, imf, unicef, derzhstat; 405 comext; 404 ecb, bundesbank; 400 insee, worldbank; 500 ilo). Today `hub.py` drops the `TIME_PERIOD` criterion and the bulk parser ignores `TimeRange`. Plan in `tasks/todo-description-timerange.md`, phase 2.
- **ISTAT IP block reproduced during the session**: the SDMX host stopped answering at TCP level (connect timeout, while `www.istat.it` and Eurostat answered) after the main session and two subagents queried it in the same minutes, each within the 13 s limiter. A router restart (new IP) cleared it. Lesson in `tasks/lessons.md`.

## 2026-08-28 - v0.24.0 - fix: hide Eurostat `$DV_` bookmarks from the catalogue (#54, part 1)
...
- **Semantic search no longer truncates its result list.** `semantic_search` sliced the top `n * 2` candidates and only then dropped ids missing from the catalogue, so it returned fewer than `n` results when more than half that window was missing. The window was sized for invalid datasets; hiding the bookmarks makes the case reachable, since an index built before this release still carries all 548. It now ranks the full array and stops at `n` visible hits — exact instead of heuristic, and free, since the `argsort` already ran over every row.
...
- Four review findings on the PR, all valid, all fixed before merge. `--page 1` slipped through the new check — the value equals the default, so only `ctx.get_parameter_source()` can tell "not given" from "given as 1" (compared via `ParameterSource.name`, since `click` is not a declared dependency and a test enforces that). The "no index" hint named the provider only when `--provider` was passed on the command line, leaving `OPENSDMX_PROVIDER` and the default with an unusable message; it now uses `_provider_cache_key()`, the value that determines the index path. The README's "rebuilding reproduces the same index" was overstated: only the *model* is fixed within a release, while `build_embeddings` re-reads the catalog, category tree and descriptions as they are at that moment. And `SKILL.md` suggested probing for an index with `search --semantic`, which hits `_check_ollama()` first — a stopped server and a missing index were indistinguishable to the agent, so the guidance now names both errors and routes back to the tree-first path either way.
- Gate: ruff clean, mypy strict clean on 15 files, 385 tests green (6 new, covering the rejected flag combinations including the explicit `--page 1`). `skills/sdmx-explorer/` updated (SKILL.md + `references/thematic-tree.md` + `references/istat-flow.md`).
