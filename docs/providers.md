...
| `annotations` | dict | absent | SDMX annotations the provider consumes, as `{stable_key: {type, value}}` where `value` is `text`, `title`, or `presence`. The stable key is the code's handle (e.g. `keywords`, `metadata_url`); the annotation `type` string and the field to read are data. Read via one single-pass reader; no per-provider branching |
...
|---|---|---|
| `df_keywords` | `LAYOUT_DATAFLOW_KEYWORDS` | keyword text, folded into embeddings |
